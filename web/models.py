...
class ActiveBaseModel(models.Model):
    ...
    class Meta:
        abstract = True #这样在创建表的时候这个表就不会被创建


class Adminstrator(ActiveBaseModel):
    """管理员"""
...
class Level(ActiveBaseModel):
    ...
    precent = models.IntegerField(verbose_name="折扣", help_text='填入0-100整数表示百分比，例如90，表示90%')
...
class Customer(ActiveBaseModel):
    ...
    creator = models.ForeignKey(verbose_name="创建者", to="Adminstrator",on_delete=models.CASCADE)
    create_date = models.DateTimeField(verbose_name="创建日期", auto_now_add=True)


class PricePolicy(models.Model):
    """价格策略（原价，后续可以根据级别的不同做不同的折扣）"""
...
class Order(ActiveBaseModel):
    ...
    create_date = models.DateTimeField(verbose_name="创建日期", auto_now_add=True, null=True, blank=True)
...
class TransactionRecord(ActiveBaseModel):
    ...
    creator = models.ForeignKey(verbose_name="管理员",to="Adminstrator",on_delete=models.CASCADE,null=True,blank=True)
